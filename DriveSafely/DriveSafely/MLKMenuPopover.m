...
@property UIImageView *menuPointerView;
...
- (id)initWithFrame:(CGRect)frame menuItems:(NSArray *)aMenuItems
{
    ...
    if (self)
    {
        ...
        _menuPointerView = [[UIImageView alloc] initWithFrame:MENU_POINTER_RECT];
        _menuPointerView.image = [UIImage imageNamed:@"options_pointer"];
        ...
        _bgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Menu_PopOver_BG"]];
        menuItemsTableView.backgroundView = _bgView;
...
- (void)showInView:(UIView *)view
{
    ...
    [UIView animateWithDuration:ANIMATION_DURATION
                     animations:^{
                         ...
                         _bgView.frame = CGRectMake(_bgView.frame.origin.x, _bgView.frame.origin.y + 400, _bgView.frame.size.width, _bgView.frame.size.height);
                     }
...
- (void)addSeparatorImageToCell:(UITableViewCell *)cell
{
    _separatorImageView = [[UIImageView alloc] initWithFrame:SEPERATOR_LINE_RECT];
    [_separatorImageView setImage:[UIImage imageNamed:@"DefaultLine"]];
    _separatorImageView.opaque = YES;

    [cell.contentView addSubview:_separatorImageView];
...
- (void)layoutUIForInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation
{
    ...
    _menuPointerView = (UIImageView *)[self.containerButton viewWithTag:MENU_POINTER_TAG];
    UITableView *menuItemsTableView = (UITableView *)[self.containerButton viewWithTag:MENU_TABLE_VIEW_TAG];
    
    if( landscape )
    {
        _menuPointerView.frame = CGRectMake(_menuPointerView.frame.origin.x + LANDSCAPE_WIDTH_PADDING, _menuPointerView.frame.origin.y, _menuPointerView.frame.size.width, _menuPointerView.frame.size.height);
